...
Commencions par des quqlques définitions pour le maillage sur un floe. Pour définir un réseau de ressort $\tau$, on défini l'ensemble de ses points $\tau_0$ disposées en forme de grille. 
$$
\tau_0 = \Omega \cap \mathbb{Z}^2
$$
De facon similaire, l'ensemble des arrete se nomme $\tau_1$, et l'ensemble des cellules $\tau_2$. Le réseau $\tau$ est obtenu à partir de $\tau_0$ en tracant les cotés de chaque carrés ; à la frontière, on trace les diagonales des carrés incomplets. On note $W (\tau, \Rdeux)$ l’ensemble des fonctions de $\tau_0$ dans $\Rdeux$. On définit également deux triangulations de $\omega$ à partir de $τ$, en prenant respectivement les diagonales des carrés dans les directions $e_x + e_y$ et $e_x − e_y$. On note ces triangulations $\tilde{\tau}$ et $\hat{\tau}$ respectivement. 
